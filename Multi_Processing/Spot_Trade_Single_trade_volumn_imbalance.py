...
def process_data(date, symbol, period):
    ...
    buy_side = file[file['side'] == 'buy'][['volumn']].groupby(pd.Grouper(freq='1s')).sum()
    sell_side = file[file['side'] == 'sell'][['volumn']].groupby(pd.Grouper(freq='1s')).sum()
    ...
    buy_side = buy_side[['volumn']].rolling(window=period, min_periods=1).sum()
    sell_side = sell_side[['volumn']].rolling(window=period, min_periods=1).sum()
    result = (buy_side / sell_side).shift(1).rename(columns={'volumn': 'feature'})
