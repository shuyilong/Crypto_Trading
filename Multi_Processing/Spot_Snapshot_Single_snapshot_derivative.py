...
def process_data(date, symbol, period, n, data_type, target_data):
    ...
    column_mapping = {"ask_price": "asks[{}].price", "bid_price": "bids[{}].price",\
                      "ask_amount": "asks[{}].amount", "bid_amount": "bids[{}].amount","middle_price": "middle_price"}
    col = column_mapping.get(target_data, None)
    if col is not None:
        col = col.format(n)

    file = file[[col]].groupby(pd.Grouper(freq='1s')).mean()
    ...
    file['deri'] = file[col]/file[col].shift(1)

    rolling = file['deri'].rolling(window=period, min_periods=1)
