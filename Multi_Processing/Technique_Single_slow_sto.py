...
def process_data(date, symbol, period):
    ...
    file['period_high'] = file[['middle_price']].rolling(window = period, min_periods=2).max().shift(1)
    file['period_low'] = file[['middle_price']].rolling(window = period, min_periods=2).min().shift(1)
    file['feature'] = 100 * (file['middle_price'].shift(1) - file['period_low'])/ \
                      (file['period_high'] - file['period_low'] + 0.00000000001)
