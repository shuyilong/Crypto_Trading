...
def process_data(date, symbol, period):
    ...
    ask_side = file[file['side'] == 'ask'][['volumn']].groupby(pd.Grouper(freq='1s')).sum()
    bid_side = file[file['side'] == 'bid'][['volumn']].groupby(pd.Grouper(freq='1s')).sum()
    ...
    ask_side = ask_side[['volumn']].rolling(window=period, min_periods=1).sum()
    bid_side = bid_side[['volumn']].rolling(window=period, min_periods=1).sum()
    result = (ask_side / bid_side).shift(1).rename(columns={'volumn': 'feature'})
